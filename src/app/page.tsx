  ...
  async function handleArchive(id: string, isArchived: boolean) {
    try {
      ...
      if (res.ok) {
        ...
        const error = await res.json()
        alert(`Failed to update archive status: ${error.error || 'Unknown error'}`)
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-50 select-none flex flex-col">
      <header className="sticky top-0 bg-gradient-to-r from-gray-50 to-gray-100 border-b border-gray-200 z-10 shadow-sm">
        ...
        <div className="max-w-5xl mx-auto px-6 mt-4 flex flex-wrap gap-2 items-center">
          ...
          <button
            ...
            title={showFavoritesOnly ? "Show all prompts" : "Show only favorite prompts"}
          >
          ...
          <button
            ...
            title={showArchivedOnly ? "Show all prompts" : "Show only archived prompts"}
          >
      ...
      <main className="flex-1 max-w-5xl mx-auto px-6 py-6">
        ...
        {!loading && prompts.length === 0 && (
          <div className="col-span-full text-center text-gray-500 mt-10">No prompts yet. Create one.</div>
        )}
        {prompts.map(p => (
          <PromptCard 
            key={p.id} 
        ...
        {!loading && prompts.length > 0 && (
          <div className="mt-8 flex flex-col items-center gap-4">
            ...
            <div className="text-sm text-gray-500">
              Showing {prompts.length} {stats && `of ${stats.totalPrompts}`} prompts
            </div>
      ...
      {editingPromptId === 'new' ? (
        <NewPromptDialog 
          onCreated={() => { load(); loadStats(); setEditingPromptId(null); }} 
        ...
        <EditPromptDialog 
          ...
          onClose={() => setEditingPromptId(null)}
          onUpdated={() => { load(); loadStats(); setEditingPromptId(null); }}
        />
      ) : null}
        ...
        <div className="max-w-5xl mx-auto px-6 py-6">
          <div className="flex flex-col items-center gap-4">
            ...
            <div className="flex items-center gap-2 text-gray-600">
              ...
              <span className="text-xs text-gray-500">Secure prompt management</span>
            </div>
            ...
            {stats && (
              <div className="flex flex-wrap justify-center items-center gap-4 sm:gap-6">
                <div className="flex items-center gap-1.5 text-sm">
                  <span className="text-gray-400">📝</span>
                  <span className="font-medium text-gray-700">{stats.totalPrompts}</span>
                  <span className="text-gray-500 text-xs">prompts</span>
                </div>
                <div className="hidden sm:block w-px h-4 bg-gray-300"></div>
                <div className="flex items-center gap-1.5 text-sm">
                  <span className="text-red-400">♥</span>
                  <span className="font-medium text-gray-700">{stats.favoritePrompts}</span>
                  <span className="text-gray-500 text-xs">favorites</span>
                </div>
                <div className="hidden sm:block w-px h-4 bg-gray-300"></div>
                <div className="flex items-center gap-1.5 text-sm">
                  <span className="text-blue-400">📦</span>
                  <span className="font-medium text-gray-700">{stats.archivedPrompts}</span>
                  <span className="text-gray-500 text-xs">archived</span>
                </div>
                <div className="hidden sm:block w-px h-4 bg-gray-300"></div>
                <div className="flex items-center gap-1.5 text-sm">
                  <span className="text-purple-400">🏷️</span>
                  <span className="font-medium text-gray-700">{stats.totalTags}</span>

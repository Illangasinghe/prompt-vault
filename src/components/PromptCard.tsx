  ...
  return (
    <div 
      className={`rounded-2xl bg-white shadow-sm p-4 border hover:shadow-md transition-all duration-200 relative group cursor-pointer select-none ${
        isArchived ? 'border-gray-200 bg-gray-50' : 'border-gray-200 hover:border-gray-300'
    ...
    >
      ...
      <div className={`absolute top-2 right-2 flex gap-1.5 transition-opacity ${showActions ? 'opacity-100' : 'opacity-0'}`}>
        <button
          onClick={(e) => { e.stopPropagation(); onFavorite(id, !isFavorite) }}
        ...
        <button
          onClick={(e) => { e.stopPropagation(); onArchive(id, !isArchived) }}
          className={`p-2 rounded-lg text-base transition-colors cursor-pointer select-none ${
            isArchived 
      ...
      {/* Content */}
      <div className={`font-medium line-clamp-1 pr-20 pb-12 ${(isFavorite || isArchived) ? 'pl-6' : ''} ${isArchived ? 'opacity-60' : ''}`}>{title}</div>
      <p className={`text-sm text-gray-500 line-clamp-3 mt-1 whitespace-pre-wrap mb-3 ${isArchived ? 'opacity-60' : ''}`}>{body}</p>
      
      {/* Tags */}
      {tags.length > 0 && (
        <div className="flex flex-wrap gap-1 mb-3">
          {tags.map(t => (
            <span key={t} className={`text-xs px-2 py-0.5 rounded-full transition-colors ${
              isArchived 
